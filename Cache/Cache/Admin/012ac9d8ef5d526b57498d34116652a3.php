...
//--></script></head><body width="100%"><div id="loader" ><?php echo L('load_page');?></div><div id="result" class="result none"></div><div class="mainbox"><?php if(empty($_GET['isajax'])): ?><div id="nav" class="mainnav_title"><div id="lang"><?php if(APP_LANG): parse_str($_SERVER['QUERY_STRING'],$urlarr); unset($urlarr[l]); $url='index.php?'.http_build_query($urlarr); if(is_array($Lang)): $i = 0; $__LIST__ = $Lang;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$langvo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($url); ?>&l=<?php echo ($langvo["mark"]); ?>" <?php if($langvo[mark]==$_SESSION['YP_lang']): ?>class="on"<?php endif; ?>><?php echo ($langvo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div><ul><a href="<?php echo U($nav[bnav][model].'/'.$nav[bnav][action],$nav[bnav][data]);?>"><?php echo ($nav["bnav"]["name"]); ?></a>|
	...
	</script><?php endif; ?><form name="myform" id="myform" action="" method="post"><div class="table-list"><table width="100%" cellspacing="0"><thead><tr><th width="40">ID</th><th align="left"><?php echo L(NAME);?></th><th  width="60" align="left"><?php echo L(width);?></th><th  width="60" align="left"><?php echo L(height);?></th><th  width="60" align="center"><?php echo L(STATUS);?></th><th width="180"   align="center"><?php echo L('manage');?></th></tr></thead><tbody><?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr><td align="center"><?php echo ($vo['id']); ?></td><td align="left"><?php echo ($vo['name']); ?></td><td align="left"><?php echo ($vo['width']); ?></td><td align="left"><?php echo ($vo['height']); ?></td><td align="center"><?php if(($vo['status']) == "1"): echo L('enable'); else: echo L('disable'); endif; ?></td><td align="center"><a href="<?php echo U('Slide/picmanage',array(fid=>$vo['id']));?>"><?php echo L('PICMANAGE');?></a> | 
			<a href="<?php echo U('Slide/edit',array(id=>$vo['id']));?>"><?php echo L('edit');?></a> | <a href="javascript:confirm_delete('<?php echo U('Slide/delete',array(id=>$vo['id']));?>')"><?php echo L('delete');?></a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div></form><div id="pages" class="page"><?php echo ($page); ?></div></body></html></body></html>