...
//--></script></head><body width="100%"><div id="loader" ><?php echo L('load_page');?></div><div id="result" class="result none"></div><div class="mainbox"><?php if(empty($_GET['isajax'])): ?><div id="nav" class="mainnav_title"><div id="lang"><?php if(APP_LANG): parse_str($_SERVER['QUERY_STRING'],$urlarr); unset($urlarr[l]); $url='index.php?'.http_build_query($urlarr); if(is_array($Lang)): $i = 0; $__LIST__ = $Lang;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$langvo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($url); ?>&l=<?php echo ($langvo["mark"]); ?>" <?php if($langvo[mark]==$_SESSION['YP_lang']): ?>class="on"<?php endif; ?>><?php echo ($langvo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div><ul><a href="<?php echo U($nav[bnav][model].'/'.$nav[bnav][action],$nav[bnav][data]);?>"><?php echo ($nav["bnav"]["name"]); ?></a>|
	...
	</script><?php endif; ?><div class="table-list"><table cellpadding=0 cellspacing=0  width="100%"><thead><tr><th align="left"><?php echo L(file_name);?></th><th align="left" style="width:8%"><?php echo L(file_size);?></th><th style="width:18%"><?php echo L(file_time);?></th><th align="center" style="width:15%"><?php echo L(manage);?></th></tr></thead><tbody><?php if(is_array($templates)): $i = 0; $__LIST__ = $templates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($vo["filename"]); ?></td><td><?php echo ($vo["filesize"]); ?></td><td  align="center" ><?php echo (todate($vo["filemtime"],'Y-m-d H:i:s')); ?></td><td  align="center" ><a href="<?php echo U($module_name.'/edit?file='.$vo[filename]);?>"><img src="./Public/Images/edit.gif"></a><a href="javascript:confirm_delete('<?php echo U($module_name.'/delete?file='.$vo[filename]);?>')"><img src="./Public/Images/delete.gif"></a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div></body></html>