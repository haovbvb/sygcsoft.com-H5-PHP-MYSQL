...
class Tree extends Think {
	...
	function get_nav($bid,$maxlevel,$effected_id='navlist',$style='filetree ' ,$homefont='',$recursion=FALSE ,$child='',$enhomefont='',$lang='') {
		...
		if($homefont) $homefont='<li id="nav_0"><a href="'.URL().'" title="'.L(HOME_FONT).'" >'.L(HOME_FONT).''.$indexen.'</a></li>';
		...
		if(!$recursion)	$this->ret .='<ul class="nav navbar-nav navbar-right">'.$homefont;
        foreach($child as $id=>$a) {
        	@extract($a);
			if(!$this->level){	
			...
			//$this->ret .= $recursion ? '<ul><li id="fat-menu" class="dropdown">' : '<li id="fat-menu" class="dropdown">';
            $recursion = FALSE;
			if($enhomefont){
				$enzm = $enname ? '<em>'.$enname.'</em>' :  '<em>'.$catdir.'</em>';
			}
            if($ischild && $level < $this->level){
				$this->ret .= '<a href="'.$url.'" title="'.$catname.'"><span class="fd">'.$catname.'</span>'.$enzm.'</a>';
                $this->get_nav($id,$maxlevel,$effected_id,$style,'',TRUE,$ischild,$enhomefont,$lang);
            } else {
			   $this->ret .= '<a href="'.$url.'" title="'.$catname.'">'.$catname.''.$enzm.'</a>';
            }
